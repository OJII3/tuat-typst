...
if exist %TEMP_DIR% (
    echo %TEMP_DIR% is already exist and delete this to continue
    rmdir "%TEMP_DIR%" /s
)
if exist %TEMP_DIR% (
    echo Canceled
...
if %ERRORLEVEL% NEQ 0 (
    echo [Error] filed to copy directory from %TEMP_DIR% to %DATA_DIR%
    exit /b 1
)
...
rmdir /s /q "%TEMP_DIR%"
if %ERRORLEVEL% NEQ 0 (
    echo [info] filed to clean up, but install finished successfully 
)
